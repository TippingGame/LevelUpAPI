...
// ImageStorageConfig configures S3-compatible storage for asynchronous image results.
...
// MissingCredentialKeys returns the configuration keys required by
// IsConfigured that are still empty.
...
type GatewayConfig struct {
	...
	// OpenAI/Codex 上游等待响应头的超时时间（秒），0 表示无超时
...
type GatewayOpenAIWSConfig struct {
	...
	// IngressInterTurnIdleTimeoutSeconds: 客户端完成一轮后允许等待下一轮消息的最长空闲时间；0 表示关闭。
...
type GatewaySchedulingConfig struct {
	...
	// PreferSoonestReset prioritizes accounts whose current quota window resets first.
...
func load(allowMissingJWTSecret bool) (*Config, error) {
	...
	// Viper's slice decoding is not reliable for comma-separated environment
	// values, and an explicitly empty environment value must override a YAML
	// value. Parse these two compatibility settings before normalizing them.
...
func setDefaults() {
	...
	// Image storage (async image task result offload to S3-compatible storage)
	...
	// Register credential keys as well so env-only deployments are decoded.
