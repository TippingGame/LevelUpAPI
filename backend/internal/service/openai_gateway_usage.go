...
func (s *OpenAIGatewayService) calculateOpenAIRecordUsageCost(
	...
	tokens UsageTokens,
	serviceTier string,
	longContextBillingEnabledOpt ...bool,
) (*CostBreakdown, error) {
	var billingModels []string
	switch value := billingModelsInput.(type) {
	case []string:
		billingModels = append([]string(nil), value...)
	case string:
		billingModels = []string{value}
	case nil:
		billingModels = nil
	default:
		return nil, fmt.Errorf("unsupported OpenAI billing model input %T", billingModelsInput)
	}
	longContextBillingEnabled := true
	if len(longContextBillingEnabledOpt) > 0 {
		longContextBillingEnabled = longContextBillingEnabledOpt[0]
	}
