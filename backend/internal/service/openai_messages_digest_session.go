...
func openAICompatAnthropicDigestNamespace(account *Account, apiKeyID int64) string {
	if account == nil || account.ID <= 0 {
		return ""
	}
	return fmt.Sprintf("%d|%d|", account.ID, apiKeyID)
}

func (s *OpenAIGatewayService) findOpenAICompatAnthropicDigestPromptCacheKey(account *Account, apiKeyID int64, digestChain string) (promptCacheKey string, matchedChain string) {
	if s == nil || digestChain == "" {
		return "", ""
	}
	ns := openAICompatAnthropicDigestNamespace(account, apiKeyID)
	if ns == "" {
		return "", ""
	}
	chain := digestChain
...
func (s *OpenAIGatewayService) bindOpenAICompatAnthropicDigestPromptCacheKey(account *Account, apiKeyID int64, digestChain, promptCacheKey, oldDigestChain string) {
