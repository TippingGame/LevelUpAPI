...
func (s *TokenRefreshService) refreshWithRetryWithRateGate(
	...
	for attempt := 1; attempt <= maxRetries; attempt++ {
		...
		// Non-retryable credential failures are cooled down first. A repeated
		// failure crosses the threshold and becomes a durable account error.
		if isNonRetryableRefreshError(err) {
			...
			if !isGrokOAuth && s.deferNonRetryableRefreshError(ctx, account, err) {
				return err
			}
			...
			var setErr error
			writeCtx, cancelWrite := rateLimitStateContext(ctx)
			defer cancelWrite()
			if isGrokOAuth {
			...
			if account.Type == AccountTypeOAuth && (!isGrokOAuth || persistentlyBlocked) {
				...
				} else if invalidateErr := s.cacheInvalidator.InvalidateToken(writeCtx, account); invalidateErr != nil {
					cacheInvalidationFailed = true
	...
	reason := strings.TrimSuffix(tokenRefreshRetryExhaustedReasonPrefix, ":")
	if lastErr != nil {
...
func (s *TokenRefreshService) deferNonRetryableRefreshError(ctx context.Context, account *Account, refreshErr error) bool {
	if s == nil || s.accountRepo == nil || account == nil || refreshErr == nil || !account.IsOAuth() || account.IsGrokOAuth() {
		return false
	}
	if isMissingRefreshTokenRefreshError(refreshErr) {
		return false
	}
	if !shouldApplyLocalErrorState(account, tokenRefreshNonRetryableStatusCode) {
		return false
	...
	message := fmt.Sprintf("Token refresh non-retryable cooldown (%d/%d): %s", count, tokenRefreshNonRetryableThreshold, logredact.RedactText(refreshErr.Error()))
	...
	if err := s.accountRepo.SetTempUnschedulable(writeCtx, account.ID, until, reason); err != nil {
		slog.Warn("token_refresh.non_retryable_temp_unschedulable_failed", "account_id", account.ID, "error", err)
	...
	s.notifyAccountSchedulingBlocked(account, until, tokenRefreshNonRetryableKeyword)
	return true
}
...
func isMissingRefreshTokenRefreshError(err error) bool {
	...
	return strings.Contains(message, "no refresh token available") || strings.Contains(message, "empty refresh_token")
}
...
func (s *TokenRefreshService) postRefreshActions(ctx context.Context, account *Account) {
	...
	if account.TempUnschedulableUntil != nil && time.Now().Before(*account.TempUnschedulableUntil) {
		writeCtx, cancelWrite := rateLimitStateContext(ctx)
		if clearErr := s.accountRepo.ClearTempUnschedulable(writeCtx, account.ID); clearErr != nil {
...
func (s *TokenRefreshService) postRefreshStateSync(ctx context.Context, account *Account) {
	...
	if s.cacheInvalidator != nil && account.Type == AccountTypeOAuth {
		if err := s.cacheInvalidator.InvalidateToken(writeCtx, account); err != nil {
	...
	s.syncSchedulerAccount(writeCtx, account)
}
...
// IsNonRetryableRefreshError exposes the refresh classifier to manual admin flows.
func IsNonRetryableRefreshError(err error) bool {
...
func (s *TokenRefreshService) ensureOpenAIPrivacy(ctx context.Context, account *Account) bool {
	...
	slog.Info("token_refresh.privacy_mode_set", "account_id", account.ID, "privacy_mode", mode)
	return true
}
