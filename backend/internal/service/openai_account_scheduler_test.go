...
func TestOpenAIGatewayService_SelectAccountForModelWithExclusions_UsesPublicSharePriorityForConsumer(t *testing.T) {
	ownerID := int64(42)
	consumerID := int64(99)
	groupID := int64(10103)
	privatePriority := 1
	...
	require.Equal(t, sharedPro.Priority, sharedPro.EffectivePriorityForRequest(ctx))
	require.Equal(t, groupedSharedPro.Priority, groupedSharedPro.EffectivePriorityForRequest(ctx))
	...
	require.NoError(t, err)
	require.NotNil(t, account)
	require.Equal(t, standard.ID, account.ID)
}

func TestOpenAIGatewayService_SelectAccountWithScheduler_AdvancedHonorsPublicPriorityForSharedProConsumer(t *testing.T) {
	ownerID := int64(42)
	consumerID := int64(99)
	groupID := int64(10104)
	privatePriority := 50
	...
	cfg.Gateway.OpenAIWS.SchedulerScoreWeights.Load = 3
	cfg.Gateway.OpenAIWS.SchedulerScoreWeights.Queue = 3
	cfg.Gateway.OpenAIWS.SchedulerScoreWeights.ErrorRate = 0
	cfg.Gateway.OpenAIWS.SchedulerScoreWeights.TTFT = 0
	concurrencyCache := schedulerTestConcurrencyCache{
		loadMap: map[int64]*AccountLoadInfo{
			sharedPro.ID: {AccountID: sharedPro.ID, LoadRate: 99, WaitingCount: 10},
			backup.ID:    {AccountID: backup.ID, LoadRate: 0, WaitingCount: 0},
		},
		acquireResults: map[int64]bool{
			sharedPro.ID: true,
			backup.ID:    true,
	...
	svc := &OpenAIGatewayService{
		accountRepo:        schedulerTestOpenAIAccountRepo{accounts: []Account{sharedPro, backup}},
		cache:              &schedulerTestGatewayCache{},
		cfg:                cfg,
		rateLimitService:   newOpenAIAdvancedSchedulerRateLimitService("true"),
	...
	require.NotNil(t, selection)
	require.NotNil(t, selection.Account)
	require.Equal(t, sharedPro.ID, selection.Account.ID)
	require.Equal(t, openAIAccountScheduleLayerLoadBalance, decision.Layer)
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_SessionStickyModelMismatchClears(t *testing.T) {
	ctx := context.Background()
	groupID := int64(10109)
	staleSticky := Account{
		ID:          21201,
	...
	staleSticky = openAITestAccountWithGroupIfUnset(staleSticky, groupID)
	backup = openAITestAccountWithGroupIfUnset(backup, groupID)
	cache := &schedulerTestGatewayCache{
		sessionBindings: map[string]int64{
			"openai:session_hash_model_mismatch": staleSticky.ID,
		},
	}

	svc := &OpenAIGatewayService{
		accountRepo:        schedulerTestOpenAIAccountRepo{accounts: []Account{staleSticky, backup}},
		cache:              cache,
		cfg:                &config.Config{},
		rateLimitService:   newOpenAIAdvancedSchedulerRateLimitService("true"),
		concurrencyService: NewConcurrencyService(schedulerTestConcurrencyCache{}),
	...
	require.Equal(t, backup.ID, cache.sessionBindings["openai:session_hash_model_mismatch"])
	if selection.ReleaseFunc != nil {
		selection.ReleaseFunc()
	}
}
...
func TestDefaultOpenAIAccountScheduler_RequirePrivacySetSkipsUnreadyAccountWithoutDisabling(t *testing.T) {
