...
func (s *OpenAIGatewayService) ForwardGrokMedia(
	...
	targetURL, err := buildGrokMediaURL(account, s.cfg, endpoint, requestID, s.settingService)
	if err != nil {
		return nil, err
	}
...
func (s *OpenAIGatewayService) forwardGrokMediaVideoContent(
	...
	statusURL, err := buildGrokMediaURL(account, s.cfg, GrokMediaEndpointVideoStatus, requestID, s.settingService)
	if err != nil {
		return nil, err
	}
	...
	if !signedContent {
		contentURL, err = buildGrokMediaURL(account, s.cfg, GrokMediaEndpointVideoContent, requestID, s.settingService)
		if err != nil {
			SetOpsLatencyMs(c, OpsUpstreamLatencyMsKey, time.Since(upstreamStart).Milliseconds())
			return nil, err
		}
...
func (s *OpenAIGatewayService) handleGrokMediaErrorResponse(
	...
	// 必须先更新 401/403/429/5xx 的账号状态；可配置透传规则可能提前返回。
