...
func TestRewriteSystemForNonClaudeCode(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			//   [2] Claude Code expansion prompt block (带 cache_control)
			systemArr, ok := parsed["system"].([]any)
			require.True(t, ok, "system should be an array, got %T", parsed["system"])
			require.Len(t, systemArr, 3, "system array should have billing + cc prompt + expansion prompt")
			...
			require.True(t, ok, "expansion prompt block should have cache_control")
			require.Equal(t, "ephemeral", cc["type"])
