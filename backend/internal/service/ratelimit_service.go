...
func (s *RateLimitService) CheckErrorPolicy(ctx context.Context, account *Account, statusCode int, responseBody []byte, requestedModel ...string) ErrorPolicyResult {
	...
	if account.Platform == PlatformAnthropic && isAnthropicRequestPolicyError(statusCode, responseBody, "") {
		return ErrorPolicyNone
	}
...
// persistExplicitModelRateLimitedState persists a model-scoped state selected
// by an account's explicit temp-unschedulable rule. Pool mode's default local
// state shield must not suppress an administrator-configured rule.
func (s *RateLimitService) persistExplicitModelRateLimitedState(ctx context.Context, account *Account, modelKey string, resetAt time.Time, reasons ...string) error {
	modelKey = strings.TrimSpace(modelKey)
	if account == nil || modelKey == "" {
		return nil
	}
...
func (s *RateLimitService) HandleUpstreamError(ctx context.Context, account *Account, statusCode int, headers http.Header, responseBody []byte, requestedModel ...string) (shouldDisable bool) {
	...
	if account.Platform == PlatformAnthropic && isAnthropicRequestPolicyError(statusCode, responseBody, upstreamMsg) {
		slog.Info("anthropic_request_policy_error_skipped", "account_id", account.ID, "status_code", statusCode)
		return false
	}
	...
	case 401:
		...
		if account.Type == AccountTypeOAuth {
			...
			if (account.Platform == PlatformOpenAI || account.Platform == PlatformAntigravity) && strings.TrimSpace(account.GetCredential("refresh_token")) == "" {
			...
			// 不写回 credentials，避免用请求开始时的旧快照覆盖另一个 worker 刚刷新的 refresh_token。
...
func (s *RateLimitService) handle429(ctx context.Context, account *Account, headers http.Header, responseBody []byte) {
	...
	if resetTimestamp == "" {
		...
		// Anthropic 无 reset 头的 429 不适合按 5h/7d 窗口长时间封禁，
		// 但完全不冷却会让调度器反复撞同一批账号并耗尽 failover 预算。
		// 所有账号类型统一走可配置的秒级兜底冷却；管理端可关闭。
		if account.Platform == PlatformAnthropic {
			slog.Warn("rate_limit_429_no_reset_time",
				"account_id", account.ID,
				"platform", account.Platform,
				"reason", "no rate limit reset time in headers; using short fallback cooldown")
			s.apply429FallbackRateLimit(ctx, account, "anthropic_no_reset_time")
			return
		}

		// 其他平台：没有重置时间，使用秒级默认回避，避免误伤长时间不可调度。
...
func parseAnthropicAggregateReset(headers http.Header, now time.Time) (time.Time, bool) {
	return parseAnthropicResetTimestamp(headers.Get("anthropic-ratelimit-unified-reset"), now, anthropic429SevenDayResetMaxAge)
}
...
func (s *RateLimitService) persistAnthropicFableWindowLimit(ctx context.Context, account *Account, headers http.Header) bool {
	...
	writeCtx, cancel := rateLimitStateContext(ctx)
	defer cancel()
	if err := s.accountRepo.SetModelRateLimit(writeCtx, account.ID, anthropicFableRateLimitKey, limit.resetAt); err != nil {
...
func (s *RateLimitService) HandleTempUnschedulable(ctx context.Context, account *Account, statusCode int, responseBody []byte, requestedModel ...string) bool {
	...
	if !account.IsPoolMode() && !shouldApplyLocalErrorState(account, statusCode) {
		return false
	}
	if !account.ShouldHandleErrorCode(statusCode) {
		return false
...
func (s *RateLimitService) triggerTempUnschedulable(ctx context.Context, account *Account, rule TempUnschedulableRule, ruleIndex int, statusCode int, matchedKeyword string, responseBody []byte, requestedModel ...string) bool {
	...
	if modelKey != "" && statusCode != http.StatusUnauthorized {
		if err := s.persistExplicitModelRateLimitedState(ctx, account, modelKey, until, reason); err != nil {
			slog.Warn("temp_unsched_model_rate_limit_set_failed", "account_id", account.ID, "model", modelKey, "error", err)
			markModelRateLimitRuntimeState(account, modelKey, until, reason)
			return true
		}
...
func (s *RateLimitService) HandleStreamTimeout(ctx context.Context, account *Account, model string) bool {
	...
	if !shouldApplyLocalSystemErrorState(account) {
		slog.Info("stream_timeout_pool_mode_skipped", "account_id", account.ID, "model", model)
		return false
	}
