...
func TestAdminService_CreateAccount_AnthropicOAuthAppliesPoolProtectionDefaults(t *testing.T) {
	...
	tempRules := accountRepo.created.GetTempUnschedulableRules()
	require.Len(t, tempRules, 2)
	require.ElementsMatch(t, []int{529, 503}, []int{tempRules[0].ErrorCode, tempRules[1].ErrorCode})
