...
type CodexClientRestrictionDetector interface {
	Detect(c *gin.Context, account *Account, options ...any) CodexClientRestrictionDetectionResult
}
...
func (d *OpenAICodexClientRestrictionDetector) Detect(c *gin.Context, account *Account, options ...any) CodexClientRestrictionDetectionResult {
	legacyCall := len(options) == 0
	policy := CodexRestrictionPolicy{}
	var body []byte
	if len(options) > 0 {
		if provided, ok := options[0].(CodexRestrictionPolicy); ok {
			policy = provided
		}
	}
	if len(options) > 1 {
		if provided, ok := options[1].([]byte); ok {
			body = provided
		}
	}
	...
	if !legacyCall && (reason == CodexClientRestrictionReasonMatchedUA || reason == CodexClientRestrictionReasonMatchedOriginator) {
