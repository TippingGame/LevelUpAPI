...
func applyCodexOAuthTransformWithOptions(reqBody map[string]any, opts codexOAuthTransformOptions) codexTransformResult {
	...
	// Keep the guidance as developer input and mirror it into instructions.
...
// extractSystemMessagesFromInput scans input for system/developer guidance.
// System items are normalized to developer, and their text is mirrored into
// reqBody["instructions"]. Lossless text-only system items may be omitted after
// promotion when the caller requests it. The variadic option preserves
// compatibility with older internal callers.
func extractSystemMessagesFromInput(reqBody map[string]any, omitOptions ...bool) bool {
	input, ok := reqBody["input"].([]any)
	if !ok || len(input) == 0 {
		return false
	}

	omitPromoted := len(omitOptions) > 0 && omitOptions[0]
...
// defaultCodexSynthInstructions selects the model-aware Codex base prompt for
// synthesized requests and keeps a minimal non-empty fallback.
...
// applyCodexClientMetadata adds the real installation identifier associated
// with the account. Existing metadata is preserved and never overwritten.
func applyCodexClientMetadata(reqBody map[string]any, account *Account) bool {
	...
	case map[string]string:
		...
		for metadataKey, value := range existing {
...
func filterCodexInput(input []any, preserveReferences bool) []any {
	return filterCodexInputWithOptions(input, codexInputFilterOptions{PreserveReferences: preserveReferences})
}
...
func filterCodexInputWithOptions(input []any, opts codexInputFilterOptions) []any {
	...
	for _, item := range input {
		...
		// store=false makes rs_* id replay unsafe, but encrypted_content still
		// carries cross-turn reasoning context, so keep the item and strip id.
		if typ == "reasoning" {
			newItem := make(map[string]any, len(m))
			for key, value := range m {
				if key == "id" {
		...
		if !opts.PreserveReferences {
			...
			// Replayed call-input items can carry client-side item_* IDs, but the
			// Codex upstream validates these IDs as fc*. Keep call_id/name while
			// dropping an ID that cannot refer to a real upstream object.
			if id, ok := m["id"].(string); ok && id != "" && !strings.HasPrefix(id, "fc") {
				ensureCopy()
				delete(newItem, "id")
			}
		} else if typ == "message" {
			// Message IDs have the same replay constraint, with msg* as the only
			// valid upstream prefix. Never manufacture a replacement identifier.
