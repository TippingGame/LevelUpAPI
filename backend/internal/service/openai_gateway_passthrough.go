	...
	if reqStream && c != nil && c.Request != nil {
		if timeoutHeaders := collectOpenAIPassthroughTimeoutHeaders(c.Request.Header); len(timeoutHeaders) > 0 {
			...
			if s.isOpenAIPassthroughTimeoutHeadersAllowed() {
				streamWarnLogger.Warn("OpenAI passthrough forwarded client timeout headers; upstream stream may terminate early")
			} else {
...
func shouldFailoverOpenAIPassthroughResponse(account *Account, statusCode int, details ...any) bool {
	upstreamMsg := ""
	var responseBody []byte
	if len(details) == 1 {
		responseBody, _ = details[0].([]byte)
	} else if len(details) >= 2 {
		upstreamMsg, _ = details[0].(string)
		responseBody, _ = details[1].([]byte)
	}
	...
	if isOpenAIRequestPolicyError(responseBody, upstreamMsg) {
		return false
	}
	...
	switch statusCode {
	case http.StatusUnauthorized, http.StatusPaymentRequired, http.StatusTooManyRequests, http.StatusForbidden, 529:
		return true
	default:
		return statusCode >= http.StatusInternalServerError || isOpenAIModelCapacityError(statusCode, upstreamMsg, responseBody)
	}
}
...
func (s *OpenAIGatewayService) handleFailoverErrorResponsePassthrough(
	...
	return newOpenAIUpstreamFailoverError(
		...
		body,
		upstreamMsg,
		!requestPolicyHit && !shouldDisable && account.IsPoolMode() && account.IsPoolModeRetryableStatus(resp.StatusCode),
	)
}
		...
		if data, ok := extractOpenAISSEDataLine(line); ok {
			...
			if eventType == "response.failed" {
				...
				if hit, code, msg := detectOpenAICyberPolicy(dataBytes); hit {
					...
					s.handleOpenAIResponsesStreamErrorSideEffect(
						ctx,
						account,
						resp.Header,
						dataBytes,
...
func (s *OpenAIGatewayService) handleNonStreamingResponsePassthrough(
	...
	mappedModel := ""
	if len(arguments) == 2 {
		originalModel, _ = arguments[0].(string)
		mappedModel, _ = arguments[1].(string)
	} else if len(arguments) >= 3 {
		originalModel, _ = arguments[1].(string)
		mappedModel, _ = arguments[2].(string)
	}
