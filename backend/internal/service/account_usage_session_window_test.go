...
func (r *sessionWindowSyncRepo) UpdateSessionWindow(_ context.Context, id int64, _ *time.Time, end *time.Time, _ string) error {
	if end == nil {
		return nil
	}
	return r.UpdateSessionWindowEnd(context.Background(), id, *end)
}
