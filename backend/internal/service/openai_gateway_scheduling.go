...
func (s *OpenAIGatewayService) selectBestAccount(ctx context.Context, groupID *int64, platform string, accounts []Account, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, requiredCapability OpenAIEndpointCapability, preferLowUpstreamRate bool) (*Account, bool) {
	...
	sort.SliceStable(eligible, func(i, j int) bool {
		...
		return s.isBetterAccount(ctx, a, b)
	})
...
func (s *OpenAIGatewayService) SelectAccountWithLoadAwareness(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}) (*AccountSelectionResult, error) {
	return s.selectAccountWithLoadAwareness(s.withOpenAIQuotaAutoPauseContext(ctx), groupID, sessionHash, requestedModel, excludedIDs, false, "", true, PlatformOpenAI)
}

func (s *OpenAIGatewayService) selectAccountWithLoadAwareness(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, requiredCapability OpenAIEndpointCapability, useUpstreamTokenCost bool, platforms ...string) (*AccountSelectionResult, error) {
		...
		if accountID > 0 && !isExcluded(accountID) {
			...
			if err == nil {
				...
				if !clearSticky && isOpenAICompatibleAccountEligibleForRequest(ctx, account, platform, requestedModel, requireCompact, requiredCapability) {
					...
					if account == nil {
						_ = s.deleteStickySessionAccountID(ctx, groupID, sessionHash)
					} else if !s.isOpenAIAccountProxyHealthSchedulable(ctx, account) {
						_ = s.deleteStickySessionAccountID(ctx, groupID, sessionHash)
	...
	for i := range accounts {
		...
		if !s.isOpenAIAccountProxyHealthSchedulable(ctx, acc) {
			continue
		}
...
func (s *OpenAIGatewayService) listSchedulableAccounts(ctx context.Context, groupID *int64, platforms ...string) ([]Account, error) {
	platform := PlatformOpenAI
	if len(platforms) > 0 {
		platform = platforms[0]
	}
...
func (s *OpenAIGatewayService) resolveFreshSchedulableOpenAIAccount(ctx context.Context, account *Account, requestedModel string, requireCompact bool, requiredCapability OpenAIEndpointCapability, platforms ...string) *Account {
	if account == nil {
		return nil
	}
