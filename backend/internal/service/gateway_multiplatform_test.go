...
func TestGatewayServiceTempUnscheduleRetryableStatusUsesDetachedContext(t *testing.T) {
	...
	svc := &GatewayService{
		accountRepo: repo,
		rateLimitService: &RateLimitService{
			tempUnschedCache: &runtimeTempUnschedCacheStub{},
		},
	}
	ctx, cancel := context.WithCancel(context.Background())
	cancel()

	svc.TempUnscheduleRetryableError(ctx, 44, &UpstreamFailoverError{
		StatusCode:             http.StatusTooManyRequests,
		RetryableOnSameAccount: true,
		ResponseBody:           []byte(`{"error":{"message":"quota temporarily exhausted"}}`),
	})
...
type mockSessionLimitCacheForGateway struct {
	registered []string
	counts     map[int64]map[string]struct{}
}

func (m *mockSessionLimitCacheForGateway) RegisterSession(ctx context.Context, accountID int64, sessionUUID string, maxSessions int, idleTimeout time.Duration) (bool, error) {
	m.registered = append(m.registered, sessionUUID)
	if m.counts == nil {
		m.counts = make(map[int64]map[string]struct{})
	}
	if m.counts[accountID] == nil {
		m.counts[accountID] = make(map[string]struct{})
	}
	if _, ok := m.counts[accountID][sessionUUID]; ok {
		return true, nil
	}
	if len(m.counts[accountID]) >= maxSessions {
		return false, nil
	}
	m.counts[accountID][sessionUUID] = struct{}{}
	return true, nil
}

func (m *mockSessionLimitCacheForGateway) RefreshSession(ctx context.Context, accountID int64, sessionUUID string, idleTimeout time.Duration) error {
	return nil
}

func (m *mockSessionLimitCacheForGateway) GetActiveSessionCount(ctx context.Context, accountID int64) (int, error) {
	if m.counts == nil || m.counts[accountID] == nil {
		return 0, nil
	}
	return len(m.counts[accountID]), nil
}

func (m *mockSessionLimitCacheForGateway) GetActiveSessionCountBatch(ctx context.Context, accountIDs []int64, idleTimeouts map[int64]time.Duration) (map[int64]int, error) {
	out := make(map[int64]int, len(accountIDs))
	for _, accountID := range accountIDs {
		count, _ := m.GetActiveSessionCount(ctx, accountID)
		out[accountID] = count
	}
	return out, nil
}

func (m *mockSessionLimitCacheForGateway) IsSessionActive(ctx context.Context, accountID int64, sessionUUID string) (bool, error) {
	if m.counts == nil || m.counts[accountID] == nil {
		return false, nil
	}
	_, ok := m.counts[accountID][sessionUUID]
	return ok, nil
}

func (m *mockSessionLimitCacheForGateway) GetWindowCost(ctx context.Context, accountID int64) (float64, bool, error) {
	return 0, false, nil
}

func (m *mockSessionLimitCacheForGateway) SetWindowCost(ctx context.Context, accountID int64, cost float64) error {
	return nil
}

func (m *mockSessionLimitCacheForGateway) GetWindowCostBatch(ctx context.Context, accountIDs []int64) (map[int64]float64, error) {
	return map[int64]float64{}, nil
}

type mockRPMCacheForGateway struct {
	counts map[int64]int
}

func (m *mockRPMCacheForGateway) IncrementRPM(ctx context.Context, accountID int64) (int, error) {
	if m.counts == nil {
		m.counts = make(map[int64]int)
	}
	m.counts[accountID]++
	return m.counts[accountID], nil
}

func (m *mockRPMCacheForGateway) GetRPM(ctx context.Context, accountID int64) (int, error) {
	if m.counts == nil {
		return 0, nil
	}
	return m.counts[accountID], nil
...
func (m *mockRPMCacheForGateway) GetRPMBatch(ctx context.Context, accountIDs []int64) (map[int64]int, error) {
	out := make(map[int64]int, len(accountIDs))
	for _, accountID := range accountIDs {
		out[accountID] = m.counts[accountID]
	}
	return out, nil
}
...
func (m *mockConcurrencyCache) CleanupExpiredAccountSlotKeys(context.Context) error {
	return nil
}
...
func TestGatewayService_SelectAccountWithLoadAwareness(t *testing.T) {
	...
	t.Run("最大会话限制-无稳定metadata时不使用内容hash占用会话", func(t *testing.T) {
		...
		sessionLimitCache := &mockSessionLimitCacheForGateway{}
		cfg := testConfig()
		cfg.Gateway.Scheduling.LoadBatchEnabled = true
		svc := &GatewayService{
			accountRepo:        repo,
		...
		for _, sessionHash := range []string{"content-hash-round-1", "content-hash-round-2"} {
			result, err := svc.SelectAccountWithLoadAwareness(ctx, nil, sessionHash, "claude-3-5-sonnet-20241022", nil, "", int64(0))
			require.NoError(t, err)
			require.NotNil(t, result)
			require.Equal(t, int64(1), result.Account.ID)
		}
		require.Empty(t, sessionLimitCache.registered, "fallback content hashes must not consume max_sessions slots")
	})
	...
	t.Run("最大会话限制-使用metadata session id计数并复用", func(t *testing.T) {
		...
		for _, sessionHash := range []string{"content-hash-round-1", "content-hash-round-2"} {
			result, err := svc.SelectAccountWithLoadAwareness(ctx, nil, sessionHash, "claude-3-5-sonnet-20241022", nil, metadataUserID, int64(0))
			require.NoError(t, err)
			require.NotNil(t, result)
			require.Equal(t, int64(1), result.Account.ID)
		}
		require.Equal(t, []string{"123e4567-e89b-12d3-a456-426614174000", "123e4567-e89b-12d3-a456-426614174000"}, sessionLimitCache.registered)

		otherMetadataUserID := FormatMetadataUserID(
			"bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb",
			"",
			"223e4567-e89b-12d3-a456-426614174000",
			"2.1.78",
		)
		_, err := svc.SelectAccountWithLoadAwareness(ctx, nil, "other-content-hash", "claude-3-5-sonnet-20241022", nil, otherMetadataUserID, int64(0))
		require.ErrorIs(t, err, ErrNoAvailableAccounts)
	})
	...
	t.Run("RPM限制-单账号触顶返回429语义错误", func(t *testing.T) {
		...
		svc := &GatewayService{
			...
			cfg:                cfg,
			concurrencyService: NewConcurrencyService(&mockConcurrencyCache{}),
			rpmCache:           &mockRPMCacheForGateway{counts: map[int64]int{1: 1}},
		}

		_, err := svc.SelectAccountWithLoadAwareness(ctx, nil, "", "claude-3-5-sonnet-20241022", nil, "", int64(0))
		require.ErrorIs(t, err, ErrAccountRPMExceeded)
	})
	...
	t.Run("RPM限制-多账号跳过热账号选择冷账号", func(t *testing.T) {
		...
		svc := &GatewayService{
			...
			cfg:                cfg,
			concurrencyService: NewConcurrencyService(&mockConcurrencyCache{}),
			rpmCache:           &mockRPMCacheForGateway{counts: map[int64]int{1: 1, 2: 0}},
		}
