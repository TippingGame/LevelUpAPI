...
func (s *adminServiceImpl) deleteUserWithAPIKeys(ctx context.Context, userID int64, apiKeys []APIKey) error {
	if s.apiKeyRepo != nil {
		for _, key := range apiKeys {
			...
			if err := deleteAPIKeyWithAudit(ctx, s.apiKeyRepo, key.ID); err != nil {
...
func (s *adminServiceImpl) BatchUpdateConcurrency(ctx context.Context, userIDs []int64, value int, mode string) (int, error) {
	...
	if !ok {
		return 0, errors.New("user repository does not support batch concurrency updates")
	}
	...
	switch mode {
	case "set":
		affected, err = repo.BatchSetConcurrency(ctx, cleaned, value)
	case "add":
		affected, err = repo.BatchAddConcurrency(ctx, cleaned, value)
	default:
