...
func (s *GatewayService) forwardAnthropicAPIKeyPassthroughWithInput(
	...
	for attempt := 1; attempt <= maxRetryAttempts; attempt++ {
		...
		upstreamReq, wireBody, err := s.buildUpstreamRequestAnthropicAPIKeyPassthrough(upstreamCtx, c, account, input.Body, token, input.RequestStream)
		releaseUpstreamCtx()
		if err != nil {
			return nil, err
		}
...
func (s *GatewayService) buildUpstreamRequestAnthropicAPIKeyPassthrough(
	...
	if bridgeClaudeCode && c != nil && c.Request != nil {
	...
	if bridgeClaudeCode {
		...
		applyClaudeCodeMimicHeaders(req, stream)
...
func (s *GatewayService) handleStreamingResponseAnthropicAPIKeyPassthrough(
	...
	flushDeferredEventLine := func() {
		...
		writeStreamLine(deferredEventLine)
		deferredEventLine = ""
	}
