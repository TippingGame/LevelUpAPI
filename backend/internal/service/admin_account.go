...
func revertAccountProxyFallback(ctx context.Context, repo AccountRepository, accountID int64) error {
	fallbackRepo, ok := repo.(accountProxyFallbackRepository)
	if !ok {
		return ErrAccountNotInFallback
	}
	return fallbackRepo.RevertProxyFallback(ctx, accountID)
}
...
func (s *adminServiceImpl) ListAccountsForSchedulerScoreFilter(ctx context.Context, platform, accountType, status, search string, groupID int64, privacyMode string) ([]Account, error) {
	...
	accounts, _, err := s.accountRepo.ListWithFilters(
		ctx,
		pagination.PaginationParams{Page: 1, PageSize: 100000},
		platform, accountType, status, search, "", groupID, 0, privacyMode,
	)
	return accounts, err
}
...
func (s *adminServiceImpl) CreateAccount(ctx context.Context, input *CreateAccountInput) (*Account, error) {
	...
	input.Credentials, accountExtra = applyAnthropicOAuthPoolProtectionDefaults(input.Platform, input.Type, input.Credentials, accountExtra)
	...
	if err := s.validateAccountLevelGroupBinding(ctx, account.Platform, account.Type, account.AccountLevel, account.Credentials, account.Extra, groupIDs); err != nil {
		return nil, err
	}
	...
	if account.ProxyID != nil && *account.ProxyID > 0 {
		if err := s.ensureProxyAccountCapacity(ctx, *account.ProxyID, 1); err != nil {
			return nil, err
		}
	}
...
func (s *adminServiceImpl) UpdateAccount(ctx context.Context, id int64, input *UpdateAccountInput) (*Account, error) {
	...
	if input.Concurrency != nil {
		...
		concurrency, err = NormalizeOpenAIPlusConcurrency(account.Platform, account.AccountLevel, concurrency)
		if err != nil {
			return nil, err
		}
		account.Concurrency = concurrency
	...
	if !account.IsCredentialShadow() {
		if err := s.validateAccountLevelGroupBinding(ctx, account.Platform, account.Type, account.AccountLevel, account.Credentials, account.Extra, targetGroupIDs); err != nil {
			return nil, err
		}
		if err := s.validateAccountShareGroupBinding(ctx, account, targetGroupIDs); err != nil {
			return nil, err
		}
	}
...
func (s *adminServiceImpl) BulkUpdateAccounts(ctx context.Context, input *BulkUpdateAccountsInput) (*BulkUpdateAccountsResult, error) {
	...
	if len(input.Credentials) > 0 || len(input.Extra) > 0 || input.ProxyID != nil || input.GroupIDs != nil || input.AccountLevel != nil || needMixedChannelCheck || hasLongContextBillingUpdate || input.ProbeEnabled != nil {
	...
	if input.GroupIDs != nil || input.AccountLevel != nil || len(input.Credentials) > 0 || len(input.Extra) > 0 {
		for _, account := range cachedTargets {
			...
			level = NormalizeOpenAIAccountLevel(account.Platform, level, credentials, extra)
...
func (s *adminServiceImpl) RevertAccountProxyFallback(ctx context.Context, id int64) error {
	if err := revertAccountProxyFallback(ctx, s.accountRepo, id); err != nil {
		return err
	}
