...
func TestGatewayService_AnthropicAPIKeyPassthrough_BuildRequestRejectsInvalidBaseURL(t *testing.T) {
	...
	_, _, err := svc.buildUpstreamRequestAnthropicAPIKeyPassthrough(context.Background(), c, account, []byte(`{}`), "k", false)
	require.Error(t, err)
}
...
func TestGatewayService_AnthropicAPIKeyPassthrough_BridgesClaudeCodeContinuationForUpstreamSubapi(t *testing.T) {
	...
	req, _, err := svc.buildUpstreamRequestAnthropicAPIKeyPassthrough(ctx, c, account, body, "upstream-key", true)
...
func TestGatewayService_AnthropicAPIKeyPassthrough_BridgesHeadersOnlyContinuationForUpstreamSubapi(t *testing.T) {
	...
	req, _, err := svc.buildUpstreamRequestAnthropicAPIKeyPassthrough(context.Background(), c, account, body, "upstream-key", true)
	...
	require.Equal(t, "stream", getHeaderRaw(req.Header, "x-stainless-helper-method"))
	require.Equal(t, "claude-code-20250219,interleaved-thinking-2025-05-14", getHeaderRaw(req.Header, "Anthropic-Beta"))
	debugRaw, ok := c.Get(claudeMimicDebugInfoKey)
	require.True(t, ok)
	debugLine, ok := debugRaw.(string)
	require.True(t, ok)
	require.Contains(t, debugLine, "tokenType=apikey")
	require.Contains(t, debugLine, "mimic=true")
	require.Contains(t, debugLine, `x-api-key="[redacted]"`)
	require.NotContains(t, debugLine, "upstream-key")
