...
func TestExtractOpenAIResponseIDFromJSONBytes(t *testing.T) {
	...
	require.Equal(t, "resp_nested", extractOpenAIResponseIDFromJSONBytes([]byte(`{"id":"event_123","response":{"id":"resp_nested"}}`)))
...
func TestOpenAIGatewayService_BindHTTPResponseAccount(t *testing.T) {
	...
	svc.bindHTTPResponseAccount(context.Background(), c, account, "event_ignored")
	got, err = svc.getOpenAIWSStateStore().GetResponseAccount(context.Background(), groupID, "event_ignored")
	require.NoError(t, err)
	require.Zero(t, got)
}
...
func TestOpenAISelectAccountForModelWithExclusions_StickyModelMismatchClearsSession(t *testing.T) {
	sessionHash := "session-model-mismatch"
	repo := stubOpenAIAccountRepo{
		accounts: []Account{
			{
				ID:          1,
	...
	acc, err := svc.SelectAccountForModelWithExclusions(context.Background(), nil, sessionHash, "gpt-5.2", nil)
	require.NoError(t, err)
...
func TestOpenAISelectAccountWithLoadAwareness_StickyModelMismatchClearsSession(t *testing.T) {
	sessionHash := "session-load-model-mismatch"
	groupID := int64(1)
	repo := stubOpenAIAccountRepo{
		accounts: []Account{
			{
	...
	selection, err := svc.SelectAccountWithLoadAwareness(context.Background(), &groupID, sessionHash, "gpt-5.2", nil)
	require.NoError(t, err)
...
func TestOpenAIBuildUpstreamRequestOAuthOfficialClientOriginatorCompatibility(t *testing.T) {
	...
	// ChatGPT validates originator against the final User-Agent client name.
	// Derive the pair from User-Agent and fall back to the built-in CLI identity
	// when the final User-Agent is absent or not an official Codex client.
	tests := []struct {
		name           string
		userAgent      string
		originator     string
		...
		{name: "official ua pairs originator", userAgent: "Codex Desktop/1.2.3", wantOriginator: "Codex Desktop", wantUA: "Codex Desktop/1.2.3"},
		{
			...
			wantUA:         "codex-tui/0.144.1 (Mac OS X 14.0; arm64) iTerm (codex-tui; 0.144.1)",
		},
		{name: "official originator without ua falls back", originator: "codex_vscode", wantOriginator: "codex_cli_rs", wantUA: codexCLIUserAgent},
		{name: "third-party ua falls back", userAgent: "luna/1.2.0", wantOriginator: "codex_cli_rs", wantUA: codexCLIUserAgent},
	}
