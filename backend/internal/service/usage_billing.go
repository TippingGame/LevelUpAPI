...
func buildBatchImageBalanceHoldFingerprint(c *BatchImageBalanceHoldCommand) string {
	...
	raw := fmt.Sprintf("%d|%d|%s|%0.10f|%0.10f", c.UserID, c.APIKeyID, strings.TrimSpace(c.BatchID), c.HoldAmount, c.ActualAmount)
	if payloadHash := strings.TrimSpace(c.RequestPayloadHash); payloadHash != "" {
		raw += "|" + payloadHash
	}
	sum := sha256.Sum256([]byte(raw))
...
type UsageBillingApplyResult struct {
	...
	BalanceOverdrafted   bool               // sufficient-balance guard missed and debt was still recorded
