...
func prepareOpenAIOwnedAccountOrgIdentityUniqueMigration(ctx context.Context, db migrationConnection) error {
...
func findDuplicateOpenAIOwnedAccountOrgIdentities(ctx context.Context, db migrationConnection) ([]string, error) {
	rows, err := db.QueryContext(ctx, `
		WITH identities AS (
			SELECT
				owner_user_id,
