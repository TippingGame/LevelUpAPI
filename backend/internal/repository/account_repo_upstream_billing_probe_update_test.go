...
func TestUpdateCredentialsAtomicallyClearsProbeForOpenAIAPIKeyIdentityChange(t *testing.T) {
	...
	mock.ExpectExec(regexp.QuoteMeta("INSERT INTO scheduler_outbox")).
		...
		WillReturnResult(sqlmock.NewResult(1, 1))
	mock.ExpectCommit()
	expectNoOpenAISharedPoolBindingChanges(mock, 27)
	repo := newAccountRepositoryWithSQL(client, db, nil)
...
func TestUpdateWithUpstreamBillingProbeEnabledRollsBackWhenOutboxFails(t *testing.T) {
	...
	mock.ExpectExec(`(?s)UPDATE accounts\s+SET error_since = NULL`).
		WithArgs(int64(27)).
		WillReturnResult(sqlmock.NewResult(0, 1))
...
func updatedAccountRows(id int64, extra string) *sqlmock.Rows {
	...
	for i, column := range dbaccount.Columns {
		switch column {
		case dbaccount.FieldID:
			values[i] = id
		case dbaccount.FieldCreatedAt, dbaccount.FieldUpdatedAt:
			values[i] = now
		case dbaccount.FieldName:
			values[i] = "test"
		case dbaccount.FieldAccountLevel:
			values[i] = service.AccountLevelFree
		case dbaccount.FieldPlatform:
			values[i] = service.PlatformOpenAI
		case dbaccount.FieldType:
			values[i] = service.AccountTypeAPIKey
		case dbaccount.FieldCredentials:
			values[i] = []byte(`{"api_key":"sk-test"}`)
		case dbaccount.FieldExtra:
			values[i] = []byte(extra)
		case dbaccount.FieldConcurrency, dbaccount.FieldLoadFactorPaidCeiling, dbaccount.FieldPriority:
			values[i] = int64(1)
		case dbaccount.FieldRateMultiplier:
			values[i] = float64(1)
		case dbaccount.FieldStatus:
			values[i] = service.StatusActive
		case dbaccount.FieldAutoPauseOnExpired:
			values[i] = false
		case dbaccount.FieldSchedulable:
			values[i] = true
		case dbaccount.FieldQuotaDimension:
			values[i] = service.QuotaDimensionGlobal
		}
	}
	return sqlmock.NewRows(dbaccount.Columns).AddRow(values...)
}

func expectNoOpenAISharedPoolBindingChanges(mock sqlmock.Sqlmock, accountIDs ...int64) {
	ids := "{"
	...
	ids += "}"
	mock.ExpectQuery(`(?s)WITH effective_pro_accounts AS`).
		WithArgs(ids, false).
		WillReturnRows(sqlmock.NewRows([]string{"group_ids"}).AddRow("{}"))
	mock.ExpectQuery(`(?s)WITH candidate_accounts AS`).
		WithArgs(ids).
		WillReturnRows(sqlmock.NewRows([]string{"account_ids", "group_ids"}).AddRow("{}", "{}"))
}
