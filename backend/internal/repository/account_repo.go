...
func (r *accountRepository) updateAccount(ctx context.Context, account *service.Account, explicitProbeEnabled *bool) error {
	...
	if contextTx == nil {
		if _, err := r.repairOpenAISharedPoolBindings(baseCtx, []int64{account.ID}); err != nil {
			return err
		}
...
func (r *accountRepository) updateLockedAccount(ctx context.Context, client *dbent.Client, account *service.Account, explicitProbeEnabled *bool) (*dbent.Account, error) {
	...
	updated, err := applyAccountUpdateFields(client.Account.UpdateOneID(account.ID), account).Save(ctx)
	if err != nil {
		return nil, err
	}
	if err := r.syncAccountErrorSince(ctx, account.ID, account.Status); err != nil {
...
func (r *accountRepository) UpdateCredentials(ctx context.Context, id int64, credentials map[string]any) error {
	...
	if contextTx == nil {
		if _, err := r.repairOpenAISharedPoolBindings(baseCtx, []int64{id}); err != nil {
			return err
		}
...
func (r *accountRepository) syncSchedulerGroupPlatformSnapshots(ctx context.Context, groupIDs []int64, platform string) {
	...
	for _, groupID := range uniquePositiveInt64s(groupIDs) {
		...
		for _, mode := range []string{service.SchedulerModeSingle, service.SchedulerModeForced} {
			...
			if err := r.setSchedulerSnapshot(ctx, bucket, accounts); err != nil {
		...
		if platform == service.PlatformAnthropic || platform == service.PlatformGemini {
			...
			if err := r.setSchedulerSnapshot(ctx, bucket, mixedAccounts); err != nil {
...
func (r *accountRepository) setSchedulerSnapshot(ctx context.Context, bucket service.SchedulerBucket, accounts []service.Account) error {
	token, err := r.schedulerCache.CaptureBucketWriteToken(ctx, bucket)
	if err != nil {
		return err
	}
	return r.schedulerCache.SetSnapshot(ctx, bucket, token, accounts)
}

// RevertProxyFallback switches an account back to its original proxy.
...
// ListShadowsByParent lists spark credential-shadow accounts for a parent.
...
// SetRateLimitedIfLater atomically extends an account-level rate limit.
// Concurrent Grok requests can observe different reset boundaries, so an
// older response must never shorten a later limit written by another request
// or another instance.
func (r *accountRepository) SetRateLimitedIfLater(ctx context.Context, id int64, resetAt time.Time) error {
	...
	if updated == 0 {
		// Another instance may already have written a later boundary. Refresh
		// this process' scheduler view even though no outbox event is needed.
		r.syncSchedulerAccountSnapshot(ctx, id)
		return nil
	}
	if err := enqueueSchedulerOutbox(ctx, r.sql, service.SchedulerOutboxEventAccountChanged, &id, nil, nil); err != nil {
...
func (r *accountRepository) BulkUpdate(ctx context.Context, ids []int64, updates service.AccountBulkUpdate) (int64, error) {
	...
	if rows > 0 && contextTx == nil {
		if updates.AccountLevel != nil || len(updates.Credentials) > 0 || len(updates.Extra) > 0 {
			// SQL-only repositories are used by focused mutation tests and do not
			// have enough repository state to hydrate or refresh shared-pool data.
			if r.client != nil {
				if _, err := r.repairOpenAISharedPoolBindings(baseCtx, ids); err != nil {
					return 0, err
				}
			}
		}
