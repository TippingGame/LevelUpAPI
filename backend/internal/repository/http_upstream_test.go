...
func TestApplyGrokCLIProxyHeaders(t *testing.T) {
	t.Run("uses stable identity for subscription proxy", func(t *testing.T) {
	...
	t.Run("accepts canonical supported override", func(t *testing.T) {
	...
	for _, version := range []string{"0.2.92", "0.2.93-beta.1", "0.3", "0.2.95+build.1", "0.2.95\r\nX-Injected: true"} {
	...
	t.Run("leaves credit API unchanged", func(t *testing.T) {
		t.Setenv(grokCLIVersionOverride, "0.2.95")
