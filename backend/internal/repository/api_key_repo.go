...
func (r *apiKeyRepository) GetByKeyForAuth(ctx context.Context, key string) (*service.APIKey, error) {
	m, err := r.activeQuery().
		...
		WithGroup(apiKeyAuthGroupQueryOptions).
		WithGroupRoutes(apiKeyGroupRouteQueryOptions).
		Only(ctx)
	if err != nil {
		if dbent.IsNotFound(err) {
...
func latestUsageLogIPsQuery(apiKeyIDs []int64, dialectName string) (string, []any) {
	if dialectName == dialect.Postgres {
		// 每个 Key 都走一次有界索引探测，避免对 usage_logs 的整段历史做窗口排序。
		return `
...
func apiKeyAuthGroupQueryOptions(q *dbent.GroupQuery) {
	q.Select(
		group.FieldID,
		group.FieldName,
		group.FieldPlatform,
		...
		group.FieldRequiredAccountLevel,
