...
func TestOpenAIGatewayHandlerResponses_FailoverAbortsWhenClientDisconnected(t *testing.T) {
	...
	require.Equal(t, []int64{1}, upstream.calls(), "客户端断开后不应再切换到账号 2; response: status=%d body=%s", rec.Code, rec.Body.String())
...
// 守卫：客户端在线时 failover 行为不变——切换到账号 2，并遵循当前路由游标的
// 两轮尝试上限；所有尝试都返回 520 后按耗尽返回 502。
func TestOpenAIGatewayHandlerResponses_FailoverContinuesForConnectedClient(t *testing.T) {
	...
	require.Equal(t, []int64{1, 2}, upstream.calls(), "在线客户端应正常切换账号，且单一路由耗尽后不重复整轮重放; response: status=%d body=%s", rec.Code, rec.Body.String())
