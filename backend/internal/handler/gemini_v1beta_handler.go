...
func (h *GatewayHandler) GeminiV1BetaListModels(c *gin.Context) {
	...
	for {
		...
		if !hasForcePlatform && (currentAPIKey.Group == nil || currentAPIKey.Group.Platform != service.PlatformGemini) {
			if routeCursor.skipToNext("gemini_list_models_non_gemini_group", nil, zap.Int64p("group_id", currentAPIKey.GroupID)) {
				continue
			}
		...
		if err != nil {
			lastErr = err
			if routeCursor.switchToNext(apiKey.ID, "gemini_list_models_forward_failed", nil, zap.Error(err), zap.Int64p("group_id", currentAPIKey.GroupID)) {
				continue
			}
	...
	if hasAntigravityFallback {
		c.JSON(http.StatusOK, gemini.FallbackModelsList())
		return
	}
	if lastErr != nil {
		googleError(c, http.StatusServiceUnavailable, "No available Gemini accounts: "+lastErr.Error())
		return
	}
...
func (h *GatewayHandler) GeminiV1BetaGetModel(c *gin.Context) {
	...
	for {
		...
		currentAPIKey := routeCandidate.APIKey
		if !hasForcePlatform && (currentAPIKey.Group == nil || currentAPIKey.Group.Platform != service.PlatformGemini) {
			if routeCursor.skipToNext("gemini_get_model_non_gemini_group", nil, zap.Int64p("group_id", currentAPIKey.GroupID)) {
				continue
			}
			googleError(c, http.StatusBadRequest, "API key group platform is not gemini")
			return
		...
		if err != nil {
			lastErr = err
			if routeCursor.switchToNext(apiKey.ID, "gemini_get_model_forward_failed", nil, zap.Error(err), zap.Int64p("group_id", currentAPIKey.GroupID)) {
				continue
			}
			googleError(c, http.StatusBadGateway, err.Error())
			return
...
geminiRouteLoop:
	for {
		...
		if !middleware.HasForcePlatform(c) && (currentAPIKey.Group == nil || currentAPIKey.Group.Platform != service.PlatformGemini) {
			if routeCursor.skipToNext("gemini_native_non_gemini_group", reqLog, zap.Int64p("group_id", currentAPIKey.GroupID)) {
		...
		if resolveErr != nil {
			reqLog.Info("gemini.route_subscription_resolve_failed",
			...
			status, _, message, retryAfter := billingErrorDetails(resolveErr)
			if retryAfter > 0 {
		...
		routeContext := baseRequestContext
		channelMapping, _ := h.gatewayService.ResolveChannelMappingAndRestrict(routeContext, currentAPIKey.GroupID, reqModel)
		currentModelName := reqModel
		if channelMapping.Mapped {
			currentModelName = channelMapping.MappedModel
		}

		if err := h.billingCacheService.CheckBillingEligibility(routeContext, currentAPIKey.User, currentAPIKey, currentAPIKey.Group, currentSubscription); err != nil {
		...
		if routeSessionKey != "" {
			currentSessionBoundAccountID, _ = h.gatewayService.GetCachedSessionAccountID(routeContext, currentAPIKey.GroupID, routeSessionKey)
			if currentSessionBoundAccountID > 0 {
			...
			if err := json.Unmarshal(body, &geminiReq); err == nil && len(geminiReq.Contents) > 0 {
				...
				if geminiDigestChain != "" {
					...
					if found {
						...
						currentSessionBoundAccountID = foundAccountID
						...
						if routeSessionKey == "" {
							routeSessionKey = service.GenerateGeminiDigestSessionKey(geminiPrefixHash, foundUUID)
						}
						_ = h.gatewayService.BindStickySession(routeContext, currentAPIKey.GroupID, routeSessionKey, foundAccountID)
						routeContext = service.WithPrefetchedStickySession(routeContext, foundAccountID, apiKeyGroupIDValue(currentAPIKey), h.metadataBridgeEnabled())
					} else {
						geminiSessionUUID = uuid.New().String()
						if routeSessionKey == "" {
							routeSessionKey = service.GenerateGeminiDigestSessionKey(geminiPrefixHash, geminiSessionUUID)
						}
					}
				}
			}
		...
		if h.gatewayService.IsSingleAntigravityAccountGroup(routeContext, currentAPIKey.GroupID) {
			routeContext = service.WithSingleAccountRetry(routeContext, true, h.metadataBridgeEnabled())
		}
		c.Request = c.Request.WithContext(routeContext)
		routeBody := body
		...
		for {
			...
			if err != nil {
				...
				default: // FailoverExhausted
					if fs.LastFailoverErr != nil && !streamStarted && shouldSwitchAPIKeyGroupRoute(fs.LastFailoverErr) &&
						routeCursor.switchToNext(apiKey.ID, "gemini_native_account_selection_exhausted", reqLog, zap.Int("upstream_status", fs.LastFailoverErr.StatusCode), zap.Int64p("group_id", currentAPIKey.GroupID)) {
						continue geminiRouteLoop
					}
			...
			if currentSessionBoundAccountID > 0 && currentSessionBoundAccountID != account.ID {
				reqLog.Info("gemini.sticky_session_account_switched",
					zap.Int64("from_account_id", currentSessionBoundAccountID),
			...
			} else if routeSessionKey != "" && currentSessionBoundAccountID == 0 && !cleanedForUnknownBinding && bytes.Contains(routeBody, []byte(`"thoughtSignature"`)) {
				...
				currentSessionBoundAccountID = account.ID
			} else if currentSessionBoundAccountID == 0 {
			...
			if !selection.Acquired {
				...
				if err := h.gatewayService.BindStickySession(c.Request.Context(), currentAPIKey.GroupID, routeSessionKey, account.ID); err != nil {
			...
			if account.Platform == service.PlatformAntigravity && account.Type != service.AccountTypeAPIKey {
				result, err = h.antigravityGatewayService.ForwardGemini(requestCtx, c, account, currentModelName, action, stream, routeBody, currentHasBoundSession)
			} else {
				result, err = h.geminiCompatService.ForwardNative(requestCtx, c, account, currentModelName, action, stream, routeBody)
			}
				...
				if errors.As(err, &failoverErr) {
					...
					case FailoverExhausted:
						if !streamStarted && shouldSwitchAPIKeyGroupRoute(fs.LastFailoverErr) &&
							routeCursor.switchToNext(apiKey.ID, "gemini_native_upstream_failover_exhausted", reqLog, zap.Int("upstream_status", fs.LastFailoverErr.StatusCode), zap.Int64p("group_id", currentAPIKey.GroupID)) {
							continue geminiRouteLoop
						}
						h.handleGeminiFailoverExhausted(c, fs.LastFailoverErr)
						return
			...
			h.submitUsageRecordTask(c.Request.Context(), func(ctx context.Context) {
				if err := h.gatewayService.RecordUsageWithLongContext(ctx, &service.RecordUsageLongContextInput{
					...
					logger.L().With(
						...
						zap.String("model", currentModelName),
						zap.Int64("account_id", account.ID),
