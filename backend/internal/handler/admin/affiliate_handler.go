...
// GetUserOverview returns one user's invite and rebate overview.
...
// ListRebateRecords returns usage-settlement affiliate rebate records.
...
// ListTransferRecords returns legacy affiliate quota-to-balance transfers.
...
func parseAffiliateRecordFilter(c *gin.Context, page, pageSize int) service.AffiliateRecordFilter {
	...
	if parsed := parseAffiliateRecordStartTime(c.Query("start_at"), userTZ); parsed != nil {
		filter.StartAt = parsed
	}
	if parsed := parseAffiliateRecordEndTime(c.Query("end_at"), userTZ); parsed != nil {
		filter.EndAt = parsed
	}
	return filter
}
