...
func (r *oauthEmailAffiliateRepoStub) EnsureUserAffiliateWithPolicy(ctx context.Context, userID int64, _ int, _ bool) (*service.AffiliateSummary, error) {
	return r.EnsureUserAffiliate(ctx, userID)
}
...
func (r *oauthEmailAffiliateRepoStub) ValidateAffiliateCode(ctx context.Context, code string, _ service.AffiliateCodeCycle) (*service.AffiliateSummary, error) {
	return r.GetAffiliateByCode(ctx, code)
}

func (r *oauthEmailAffiliateRepoStub) ConsumeAffiliateCode(ctx context.Context, userID int64, code string, _ service.AffiliateCodeCycle) (*service.AffiliateSummary, error) {
	inviter, err := r.GetAffiliateByCode(ctx, code)
	if err != nil {
		return nil, err
	}
	r.bindCalls = append(r.bindCalls, oauthEmailAffiliateBindCall{userID: userID, inviterID: inviter.UserID})
	return inviter, nil
}
