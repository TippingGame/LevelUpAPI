...
type ResponsesNamespaceName struct {
	Namespace string
	Name      string
}

const responsesNamespaceToolNameMaxLen = 64
...
func flattenResponsesNamespaceToolName(namespace, name string) string {
	...
	if len(full) <= responsesNamespaceToolNameMaxLen {
		return full
	}
	sum := sha256.Sum256([]byte(full))
	suffix := "__" + hex.EncodeToString(sum[:4])
	prefixLimit := responsesNamespaceToolNameMaxLen - len(suffix)
	var prefix strings.Builder
	for _, character := range full {
