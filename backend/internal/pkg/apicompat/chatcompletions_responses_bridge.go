...
// Completions request for upstreams that do not implement /v1/responses.
...
// response into a Responses response.
...
func ChatUsageToResponsesUsage(usage *ChatUsage) *ResponsesUsage {
	...
	if usage.PromptTokensDetails != nil {
		switch {
		case usage.PromptTokensDetails.CacheWriteTokens > 0:
			out.CacheCreationInputTokens = usage.PromptTokensDetails.CacheWriteTokens
		case usage.PromptTokensDetails.CacheCreationTokens > 0:
...
func chatToResponsesEvent(state *ChatCompletionsToResponsesStreamState, eventType string, template *ResponsesStreamEvent) ResponsesStreamEvent {
