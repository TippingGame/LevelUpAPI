...
func TestNormalizeSSOTokenAcceptsCookieHeaderAndStripsControls(t *testing.T) {
	...
	require.Equal(t, "raw-token", NormalizeSSOToken(" raw-\r\ntoken ; ignored=1"))
}

func TestSSODeviceFlowRejectsUntrustedRedirect(t *testing.T) {
	client := ssoDeviceClientFunc(func(req *http.Request) (*http.Response, error) {
		return ssoDeviceResponse(http.StatusFound, http.Header{"Location": {"https://example.com/steal"}}, ``), nil
	})
	_, err := ConvertSSOToBuild(context.Background(), "secret", &SSODeviceOptions{HTTPClient: client})
	require.ErrorContains(t, err, "untrusted host")
}

type ssoDeviceClientFunc func(*http.Request) (*http.Response, error)

func (f ssoDeviceClientFunc) Do(req *http.Request) (*http.Response, error) { return f(req) }
