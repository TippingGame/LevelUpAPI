export default {
  ...
  userAccounts: {
    ...
    loadFactorCreditsPreview: '已付峰值 {paid}，本次消耗 {cost}，剩余额度 {balance}',
    loadFactorCreditsInsufficient: '负载因子额度不足',
    bulkLoadFactorCreditsHint: '批量调整会按每个账号的已付峰值分别扣减额度，降低不返还。',
    ...
    importWarning: '单次最多导入 {max} 个账号。支持 Sub2API OAuth JSON、Codex-Manager ChatGPT Token JSON、OpenAI Refresh Token、Claude Session Key；API Key、URL、Upstream、Cookie 会被拒绝。',
    importWarningChoosePlatform: '请先选择导入平台。单次最多导入 {max} 个账号；API Key、URL、Upstream、Cookie 会被拒绝。',
    importWarningOpenAI: '单次最多导入 {max} 个 OpenAI 账号。Free / Plus / Team 可直接导入；Pro 可选择批量凭证导入或账号登录，但必须选择代理 IP。',
    importWarningClaude: '单次最多导入 {max} 个 Claude 账号。支持 Sub2API OAuth JSON 或 Claude Session Key。',
    importWarningGemini: '单次最多导入 {max} 个 Gemini 账号。仅支持带 Gemini 平台信息的官方 OAuth JSON。',
    importWarningAntigravity: '单次最多导入 {max} 个 Antigravity 账号。仅支持带 Antigravity 平台信息的官方 OAuth JSON。',
    importWarningGrok: '单次最多导入 {max} 个 Grok 账号。请明确选择 OAuth JSON、Refresh Token 或 Web SSO；三种凭证不会混合解析。',
    importPlatform: '导入平台',
    ...
    importPlatformHintClaude: 'Claude 导入不会使用 OpenAI 账号等级。请选择 Claude OAuth JSON 或 Claude Session Key。',
    importPlatformHintGemini: 'Gemini 导入不会使用 OpenAI 账号等级。请导入包含 platform: gemini 的 OAuth JSON。',
    importPlatformHintAntigravity: 'Antigravity 导入不会使用 OpenAI 账号等级。请导入包含 platform: antigravity 的 OAuth JSON。',
    importPlatformHintGrok: 'Grok 账号使用官方 xAI OAuth，并必须绑定可见代理。Web SSO 会由服务端通过 xAI Device Flow 转换，原始 SSO 不会保存。',
    importGrokMethod: 'Grok 导入方式',
    importGrokMethodOAuth: 'OAuth JSON',
    importGrokMethodOAuthHint: '导入含 access_token 和 platform: grok 的账号 JSON。',
    importGrokMethodRefreshToken: 'Refresh Token',
    importGrokMethodRefreshTokenHint: '每行一个 xAI refresh token，验证后创建账号。',
    importGrokMethodSSO: 'Web SSO',
    importGrokMethodSSOHint: '每行一个 Grok Web SSO key，自动转换为 Build OAuth。',
    importAccountLevel: 'OpenAI 账号等级',
    importAccountLevelHint: '仅导入 OpenAI 账号时需要选择。OpenAI 会按所选等级严格校验；无法确认等级时按 Free 处理；Pro 必须选择代理 IP。',
    importAccountLevelRequired: '请先选择 OpenAI 账号等级',
    importLevelFree: '无法确认等级时归入 Free',
    importLevelPlus: '只接受实际 Plus 账号',
    importLevelPro: '需选择代理 IP',
    importLevelTeam: '支持 JSON 导入，仅接受实际 Team 账号',
    importShareModeHint: '选择公共后，系统会在导入完成时校验账号、公共号池分组和分成策略；通过后自动绑定公共分组，未通过则保留私有默认分组并显示原因。',
    importMethod: 'Pro 导入方式',
    importMethodCredentials: '批量凭证导入',
    importMethodCredentialsHint: '粘贴 JSON 或 Refresh Token，整批账号使用所选代理。',
    importMethodOAuth: '账号登录',
    importMethodOAuthHint: '生成登录链接，完成一次 OpenAI 授权导入。',
    importOAuthOnlyHint: '请先选择代理 IP，再生成登录链接并粘贴回调结果。',
    importOAuthNamePlaceholder: '可选；留空则使用 OpenAI 邮箱',
    importProxy: '代理 IP',
    importProxyHint: '请选择一个可见且启用的代理 IP，用于需要代理的登录或导入，并绑定到导入账号。OpenAI Pro / Claude / Gemini / Antigravity / Grok 必选。',
    ...
    proxySmartPlaceholder: "示例：\n192.168.0.1:8000:用户名:密码\n用户名:密码{'@'}192.168.0.1:8000",
    proxySmartApply: '识别填入',
    proxySmartHint: '支持 socks5/http/https URL，也支持账号密码前置或冒号分隔格式。',
    proxyName: '代理名称',
    proxyNamePlaceholder: '例如：Roxy 独立 IP / 家宽代理',
    proxyNameHint: '用于在下拉框中识别该代理，仅自己可见；不填会按主机和端口自动生成。',
    ...
    importOAuthCallbackRequired: '请先生成登录链接，并粘贴包含 code 和 state 的 OpenAI 回调结果',
    importTextMode: '批量文本',
    ...
    importTextHintOpenAI: 'OpenAI Free / Plus / Team 可粘贴 OAuth JSON 或 Refresh Token；Pro 也支持导入，但必须选择代理 IP。',
    importTextHintClaude: 'Claude 可粘贴 Claude Session Key 或包含 Anthropic/Claude 平台信息的 OAuth JSON。',
    importTextHintGemini: 'Gemini 仅接受包含 Gemini 平台信息的 OAuth JSON，不接收普通 Token。',
    importTextHintAntigravity: 'Antigravity 仅接受包含 Antigravity 平台信息的 OAuth JSON，不接收普通 Token。',
    importTextHintGrokOAuth: '仅解析 Grok OAuth JSON；请包含 platform: grok 和 access_token，不会把普通文本误当作其他平台凭证。',
    importTextHintGrokRefreshToken: '每行一个 xAI Refresh Token；系统会先通过所选代理验证，再逐项创建账号并汇总失败项。',
    importTextHintGrokSSO: '每行一个 Grok Web SSO key；系统以 3 路并发完成 xAI Device Flow。原始 SSO 不会写入账号、日志或响应。',
  ...
  keys: {
    ...
    imagePlaygroundUnavailable: '功能暂未开启',
    enable: '启用',
    disable: '禁用',
    nameLabel: '名称',
    ...
    choosePlatformFirst: '选择平台后将显示该平台下可用的号池',
    selectedGroupCount: '已选 {count} 个',
    clearSelection: '清空',
    saveSelection: '保存',
    samePlatformOnly: '只能选择同一平台的号池',
    privateGroupDescription: '用户 {userId} 的 {platform} 私有订阅号池。',
    privateGroupDescriptionWithoutOwner: '{platform} 私有订阅号池。',
    statusLabel: '状态',
    selectStatus: '选择状态',
    saving: '保存中...',
    noKeysYet: '暂无 API 密钥',
  ...
  affiliate: {
    ...
    weeklyQuotaUnlimited: '不限人数（本周已邀请 {used} 人）',
    codePolicy: {
      title: '邀请码策略',
  ...
  profile: {
    ...
    receiptCode: {
      title: '收款码',
      description: '上传支付宝或微信收款二维码，图片保存到 OSS，不占用本地容器磁盘。',
      empty: '暂无图片',
  ...
  admin: {
    ...
    revenue: {
      ...
      fields: {
        ...
        balanceGtPointOneWithPendingWithdrawal: '平台用户大于 0.1 余额+待提现余额',
      ...
      shareSettlements: {
        ...
        errors: {
          ...
          REVENUE_EXPORT_TOO_LARGE: '导出结果超过 100 万行，请缩小日期或筛选条件',
    ...
    users: {
      ...
      invitePolicy: {
        ...
        weeklyLimitHint: '默认不限制邀请人数；填写大于 0 的数值才会启用每周上限。',
        autoRotateLabel: '启用每周自动换码',
        loading: '正在加载邀请码策略',
        ...
        weeklyUsageUnlimited: '本周已用 {used} / 不限',
        remaining: '剩余 {remaining} 次',
        remainingUnlimited: '不限人数'
      },
      adjustBalance: '调整余额',
    ...
    riskControl: {
      ...
      cyberPreflightHint: '开启后使用本地高风险网络滥用规则，在账号选择前拦截命中请求；需同时开启风控中心总开关。',
      mode: '全局模式',
      modePreBlock: '前置拦截',
      ...
      providerZhipuHint: '使用智谱 moderation 模型，按 PASS / REVIEW / REJECT 判断，文本会按 2000 字符分块审核。',
      baseUrl: '审核 Base URL',
      model: '模型名',
      ...
      zhipuImageUnsupported: '智谱内容安全图片审核尚未适配，当前仅支持文本审核。',
      ...
      cyberRules: {
        notice: '每行一条规则。保存后会在账号选择前本地判断；清空某个规则组表示禁用该组。',
        placeholder: '每行一条',
        count: '{count} 条',
        resetDefaults: '恢复默认规则',
        standaloneBlockMarkers: '高危独立拦截词',
        standaloneBlockMarkersHint: '命中后直接拦截，适合免杀、钓鱼页面、键盘记录等极高风险表达。',
        hardMarkers: '高危技术词',
        hardMarkersHint: '需结合攻击意图或真实目标才会拦截，降低安全讨论误伤。',
        offensiveIntentMarkers: '攻击意图词',
        offensiveIntentMarkersHint: '与高危技术词或攻击技术词组合命中，例如生成、攻击、绕过、利用。',
        credentialAbuseIntentMarkers: '凭证滥用意图词',
        credentialAbuseIntentMarkersHint: '与凭证词组合命中，例如抓取、导出、盗取、泄露。',
        techniqueMarkers: '攻击技术词',
        techniqueMarkersHint: '与攻击意图或真实目标组合命中，例如注入、RCE、爆破、扫描。',
        credentialMarkers: '凭证词',
        credentialMarkersHint: '与凭证滥用意图组合命中，例如 token、cookie、密码、密钥。',
        targetMarkers: '真实目标词',
        targetMarkersHint: '与高危技术词或攻击技术词组合命中，例如公网、生产环境、后台、登录页。',
        defensiveMarkers: '防御/授权放行词',
        defensiveMarkersHint: '用于降低误伤，例如检测、修复、防御、授权测试、靶场、CTF。',
      },
    ...
    accounts: {
      ...
      quotaDashboard: {
        ...
        issueMeta: '限流 {limited}，限额保护 {protected}，错误 {error}，禁用 {disabled}',
        accountStatus: '账号状态',
        totalShort: '总 {count}',
      ...
      credentialImportHint: '粘贴账号凭证或导入文件，后台凭证导入会校验并创建对应平台的 OAuth 账号。',
      credentialImportWarning: '该入口接收官方 OAuth 凭证：Sub2API OAuth JSON、Grok OAuth JSON、带 platform: grok 的 Refresh Token JSON、Codex-Manager ChatGPT Token JSON、OpenAI Refresh Token、Claude Session Key；API Key、自定义 URL、Upstream、Cookie 会被拒绝。管理员可通过“添加账号”创建 Grok API Key 账号。',
      dataExportConfirmMessage: '导出的数据包含账号与代理的敏感信息，请妥善保存。',
      dataExportConfirm: '确认导出',
      dataExported: '数据导出成功',
      dataExportFailed: '数据导出失败',
      dataImportTitle: '导入数据',
      dataImportHint: '上传导出的账号/代理数据包或账号凭证 JSON；支持 Grok OAuth JSON 和带 platform: grok 的 Refresh Token JSON。',
      dataImportWarning: '数据包会创建新账号与代理，凭证 JSON 会校验并创建 OAuth 账号；分组需手工绑定，请确认已有数据不会冲突。',
      dataImportFile: '数据文件',
      ...
      dataImportInvalidFile: '文件 {name} 不是受支持的数据导出格式',
      dataImportMixedFormats: '导出数据文件与账号凭证文件不能混合批量导入',
      dataImportIgnoredFiles: '已忽略 {count} 个非 JSON 文件',
      dataImportSelectedCount: '已选择 {count} 个文件',
      dataImportDropHint: '支持选择或拖拽多个 JSON 文件',
      ...
      status: {
        ...
        codexQuotaProtectedUntil: 'Codex {window} 用量达到限额，当前不参与调度，预计 {time} 自动恢复',
      ...
      openaiQuotaReset: {
        ...
        confirmMessage: '将为账号“{name}”消耗 1 次 OpenAI 重置次数。当前可用 {count} 次，确认继续？',
        confirmAction: '确认重置'
      },
      tier: {
        free: 'Free',
      ...
      openai: {
        ...
        codexQuotaLimitDesc: '仅对 OpenAI OAuth 生效。5h 或 7d 用量达到设置百分比后暂停调度，窗口重置后自动恢复；100% 表示按原始额度保护。',
      ...
      customErrorCodesHint: '仅对选中的错误码或范围停止调度',
      customErrorCodesWarning: '仅选中的错误码或范围会停止调度，其他错误将返回 500。',
      ...
      enterErrorCode: '输入错误码或范围，如 401,500-503',
      invalidErrorCode: '请输入有效的 HTTP 错误码或范围 (100-599)',
      errorCodeExists: '该错误码或范围已被选中',
      ...
      oauth: {
        ...
        grok: {
          ...
          oauthOnlyHint: '用户自有 Grok 账号仅支持 xAI 官方 OAuth，必须选择本人可见代理，并发固定为 1。',
          ...
          ssoCookieAuth: 'Web SSO 导入',
          ssoCookieDesc: '每行粘贴一个 Grok Web SSO key，服务端会完成 xAI Device Flow，并转换成官方 Build OAuth 凭据。',
          ssoCookieLabel: 'Grok Web SSO Key',
          ssoCookiePlaceholder: '每行一个 SSO key\n也支持 sso=... 或完整 Cookie 头',
          ssoCookieHint: '转换采用 3 路并发；需要时请选择对应地区代理，原始 SSO 不会保存。',
          convertingSSO: '转换中...',
          ...
          failedToConvertSSO: 'Grok Web SSO 转换失败',
          errors: {
    ...
    proxies: {
      ...
      maxAccountsBelowCurrent: '当前已绑定 {current} 个账号，上限不能低于当前绑定数，除非设置为 0',
    ...
    ops: {
      ...
      runtime: {
        ...
        requestErrorRateMaxPercentHint: '客户端请求错误率超过该值时会触发风险提示。',
    ...
    settings: {
      ...
      features: {
        ...
        riskControl: {
          ...
          cyberSessionBlockEnabledHint: '上游返回 cyber_policy 后，把同一显式会话后续请求拦在账号选择前；默认关闭。',
          cyberSessionBlockTTL: '会话屏蔽 TTL（秒）',
          cyberSessionBlockTTLHint: '建议 3600 秒。仅对 session_id、conversation_id 或 prompt_cache_key 生效。',
        },
        affiliate: {
          title: '邀请收益',
      ...
      security: {
        upstreamAllowlist: {
          title: '上游域名白名单',
          description: '追加允许作为账号 Base URL 的上游域名或 IP 直连地址。配置文件中的默认白名单仍然保留。',
          placeholder: 'naiccc.com\n*.naiccc.com\n203.0.113.10:8080',
          hint: '每行一个条目，支持域名、*.example.com、IP 或 IP:端口。不要填写协议、路径或 API Key。',
          invalid: '上游白名单格式不正确，请填写域名、*.example.com、IP 或 IP:端口。'
        }
      },
      turnstile: {
      ...
      defaults: {
        ...
        defaultAffiliateWeeklyLimit: '邀请每周可用次数',
        defaultAffiliateWeeklyLimitHint: '新用户邀请返利码每周可使用次数，0 = 不限制；仅作用于新用户创建邀请档案时初始化',
        defaultAffiliateCodeAutoRotate: '每周自动换码',
        defaultAffiliateCodeAutoRotateHint: '开启后新用户的邀请码会在每周周期重置时自动更换；关闭时邀请码长期有效',
      ...
      gatewayForwarding: {
        ...
        claudeOAuthSystemPromptInjectionHint: '对使用 Claude OAuth 账号的非 Claude Code 客户端注入可配置的 Claude Code 风格 system blocks。',
        ...
        anthropicCacheTTL1hInjectionHint: '开启后，对 Anthropic OAuth/Setup Token 请求体中已有的 ephemeral 缓存块强制写入 1h；响应 usage 默认按上游返回的 5m/1h 明细计费，账号级 TTL 计费设置优先。',
      ...
      payment: {
        ...
        tabVisibilityHint: '关闭后，用户侧充值/订阅页将不显示对应的顶部 Tab 和内容区域。',
        ...
        rechargeCenterHint: '配置用户充值/订阅页中的外部入口，用户点击后默认新窗口打开。',
        ...
        rechargeCenterUrlInvalid: '充值中心入口链接必须是有效的 http(s) 绝对地址。',
        helpText: '帮助文本',
        helpImageUrl: '帮助图片链接',
        ...
        receiptCodeOssHint: '将用户提现收款码图片保存到 S3 / OSS / COS 兼容对象存储。',
      ...
      receiptCode: {
        title: '收款码存储',
        description: '配置共享号主提现收款码图片的对象存储。保存后配置会写入系统设置并加密密钥，不需要继续维护 .env。',
        storageEnabled: '启用收款码对象存储',
        storageEnabledHint: '开启后，共享号主可以在个人资料上传支付宝或微信收款码。',
        endpoint: 'Endpoint',
        region: 'Region',
        bucket: 'Bucket',
        ...
        publicBaseUrlHint: '通常留空，系统会使用临时签名链接读取私有对象。',
        maxSize: '最大图片大小（字节）',
        presignExpire: '签名链接有效期（秒）',
        forcePathStyle: '使用路径风格请求',
        secretConfiguredPlaceholder: '已配置，留空保持不变',
        storageNote: '腾讯 COS 可填写类似 https://cos.ap-guangzhou.myqcloud.com、Region ap-guangzhou、Bucket 带 APPID 的完整名称。建议 Bucket 保持私有，并使用单独前缀保存收款码。',
      },
      balanceNotify: {
        title: '余额不足提醒',
  ...
  agentExperience: {
    title: 'LevelUpAgent 在线体验',
    subtitle: '选择你的 API 密钥和模型，在浏览器中体验专注的问答模式',
    qaMode: '问答模式',
    projectHome: '项目主页',
    downloadFull: '下载完整版',
    apiKey: 'API 密钥',
    ...
    newChat: '新对话',
    createKey: '创建 API 密钥',
    promptExplain: '解释一个复杂的技术概念',
    promptPlan: '帮我梳理一个项目实施方案',
    promptWrite: '写一个清晰可靠的代码示例',
    preparing: '正在准备在线体验',
    noKeyTitle: '先创建一个 API 密钥',
    noKeyDescription: '在线体验使用你在本站创建的密钥，不需要手动复制或填写。',
    chooseModelTitle: '选择一个模型开始体验',
    chooseModelDescription: '模型列表会根据所选密钥的可用渠道自动加载。',
    readyTitle: '今天想了解什么？',
    readyDescription: '这是 LevelUpAgent 的浏览器问答模式。输入问题，即可体验流式回答与 Markdown 展示。',
    ...
    emptyResponse: '模型没有返回可显示的内容，请重试。',
