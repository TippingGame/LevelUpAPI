        ...
        <template #cell-cost="{ row }">
          <div class="text-sm">
            ...
            <div v-if="row.account_rate_multiplier != null" class="mt-0.5 text-[11px] text-orange-500 dark:text-orange-400">
              A {{ formatUsageCost(accountBilled(row)) }}
            </div>
          </div>
        </template>
        ...
        <div class="space-y-1.5">
          ...
          <div class="mb-2 border-b border-gray-700 pb-1.5">
            ...
            <div v-if="tooltipData && tooltipData.input_cost > 0" class="flex items-center justify-between gap-4">
              ...
              <span class="font-medium text-white">{{ formatUsageCost(tooltipData.input_cost) }}</span>
            </div>
            ...
            <div v-if="tooltipData && tooltipData.output_cost > 0" class="flex items-center justify-between gap-4">
              ...
              <span class="font-medium text-white">{{ formatUsageCost(tooltipData.output_cost) }}</span>
            </div>
            ...
            <template v-if="!tooltipData?.billing_mode || tooltipData.billing_mode === BILLING_MODE_TOKEN">
              <div v-if="tooltipData && textInputTokens(tooltipData) > 0" class="flex items-center justify-between gap-4">
                ...
                <span class="font-medium text-sky-300">{{ formatTokenPricePerMillionCoins(tooltipData.input_cost, textInputTokens(tooltipData)) }} {{ t('usage.perMillionTokens') }}</span>
              </div>
              <div v-if="tooltipData && hasImageInputTokens(tooltipData)" class="flex items-center justify-between gap-4">
                ...
                <span class="font-medium text-fuchsia-300">{{ formatTokenPricePerMillionCoins(tooltipData.image_input_cost, tooltipData.image_input_tokens) }} {{ t('usage.perMillionTokens') }}</span>
              </div>
            ...
            <div v-else class="flex items-center justify-between gap-4">
              ...
              <span class="font-medium text-sky-300">{{ formatUsageCost(tooltipData.total_cost) }}</span>
            </div>
            <div v-if="tooltipData && tooltipData.cache_creation_cost > 0" class="flex items-center justify-between gap-4">
              <span class="text-gray-400">{{ t('admin.usage.cacheCreationCost') }}</span>
              <span class="font-medium text-white">{{ formatUsageCost(tooltipData.cache_creation_cost) }}</span>
            </div>
            <div v-if="tooltipData && tooltipData.cache_read_cost > 0" class="flex items-center justify-between gap-4">
              <span class="text-gray-400">{{ t('admin.usage.cacheReadCost') }}</span>
              <span class="font-medium text-white">{{ formatUsageCost(tooltipData.cache_read_cost) }}</span>
            </div>
          </div>
          <!-- Rate and Summary -->
          ...
          <div class="flex items-center justify-between gap-6">
            ...
            <span class="font-medium text-white">{{ formatUsageCost(tooltipData?.total_cost) }}</span>
          </div>
          <div class="flex items-center justify-between gap-6">
            <span class="text-gray-400">{{ t('usage.userBilled') }}</span>
            <span class="font-semibold text-green-400">{{ formatUsageCost(tooltipData?.actual_cost) }}</span>
          </div>
...
import { hasImageInputTokens, textInputTokens, hasImageInputCost } from '@/utils/imageUsage'
...
function formatUsageCost(value?: number | null): string {
  return formatGameCoins(value || 0, { minimumFractionDigits: 6, maximumFractionDigits: 6 })
}

function formatTokenPricePerMillionCoins(cost?: number | null, tokens?: number | null): string {
  if (!cost || !tokens) return formatUsageCost(0)
  return formatUsageCost((cost / tokens) * 1_000_000)
}
