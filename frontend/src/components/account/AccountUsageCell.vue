  ...
  <div ref="rootRef" v-if="showUsageWindows">
    ...
    <template v-else-if="account.platform === 'grok' && account.type === 'oauth'">
      ...
      <div v-else-if="usageInfo" class="space-y-1">
        ...
        <GrokQuotaProbeCell :account="account" :account-scope="accountScope" @probed="handleGrokProbed" />
      </div>
      <div v-else class="text-xs text-gray-400">-</div>
    </template>
...
const formatKeyCost = computed(() => {
  if (!props.todayStats) return formatGameCoins(0)
  return formatGameCoins(props.todayStats.cost)
})

const formatKeyUserCost = computed(() => {
  if (!props.todayStats || props.todayStats.user_cost == null) return formatGameCoins(0)
  return formatGameCoins(props.todayStats.user_cost)
})
