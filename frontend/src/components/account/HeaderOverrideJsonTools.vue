<template>
  ...
  <div v-if="showImportPanel" ref="importPanelRef" class="w-full space-y-2">
    ...
    <div class="flex gap-2">
      ...
      <button type="button" class="rounded-lg bg-gray-100 px-3 py-1 text-xs text-gray-600 hover:bg-gray-200 dark:bg-dark-600 dark:text-gray-400 dark:hover:bg-dark-500" @click="closeImportPanel">
        {{ t('admin.accounts.headerOverride.importJsonCancel') }}
      </button>
    </div>
    <p class="text-xs text-gray-500 dark:text-gray-400">{{ t('admin.accounts.headerOverride.importJsonHint') }}</p>
  </div>
</template>
...
// Keep the JSON example out of i18n: raw braces are parsed as placeholders by vue-i18n.
