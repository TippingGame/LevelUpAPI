...
export function useChannelMonitorFormat() {
  ...
  function providerPickerClass(p: Provider | string, active: boolean): string {
    switch (p) {
      ...
      case PROVIDER_GROK:
        return active
          ? 'border-zinc-600 bg-zinc-100 text-zinc-800 dark:border-zinc-400 dark:bg-zinc-500/15 dark:text-zinc-200'
          : 'border-gray-200 bg-white text-gray-600 hover:border-zinc-400 hover:text-zinc-800 dark:border-dark-700 dark:bg-dark-800 dark:text-gray-400 dark:hover:border-zinc-500'
      default:
        return active
