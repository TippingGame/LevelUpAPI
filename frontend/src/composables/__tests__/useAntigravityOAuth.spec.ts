...
vi.mock('@/api/accounts', () => ({
  accountsAPI: {
    generateAntigravityOAuthUrl: vi.fn(),
    exchangeAntigravityOAuthCode: vi.fn(),
    refreshAntigravityToken: vi.fn()
  }
}))
...
describe('useAntigravityOAuth.buildCredentials', () => {
  it('preserves the submitted refresh token when validation omits it', () => {
  ...
  it('prefers a rotated refresh token returned by validation', () => {
    const oauth = useAntigravityOAuth()
