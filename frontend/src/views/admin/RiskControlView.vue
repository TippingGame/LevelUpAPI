...
type SettingsTab = 'basic' | 'scope' | 'cyberRules' | 'runtime' | 'response' | 'riskThresholds' | 'retention'
...
const modelFilterOptions = computed<
  Array<{ value: ContentModerationModelFilterType; label: string; description: string }>
>(() => [
  {
    value: 'all',
...
function riskThresholdsFromConfig(
...
function buildRiskThresholdPayload(): Record<string, number> {
  ...
  for (const category of riskThresholdCategories) {
    payload[category] = Number(
      (clampPercent(configForm.thresholds[category]) / 100).toFixed(4)
    )
  }
  return payload
}
