...
本文用于维护当前二开版本：在后续官方 `Wei-Shaw/sub2api` 发布新版本时，合并官方更新，同时保留本项目的个人账号、公共账号共享、分成结算、收益管理增强、凭证导入限制等二开功能。
...
生产环境不要继续使用官方镜像 `weishaw/sub2api:latest`，否则会覆盖二开功能。必须构建并使用自己的镜像。
...
IMAGE=registry.example.com/sub2api-custom:$VERSION

docker build \
  --build-arg VERSION=$VERSION \
  --build-arg COMMIT=$COMMIT \
  -t $IMAGE \
  -t registry.example.com/sub2api-custom:latest \
  .
```
...
docker buildx create --use --name sub2api-builder || true
docker buildx build \
...
docker push registry.example.com/sub2api-custom:v0.1.122-2dev.1
docker push registry.example.com/sub2api-custom:latest
```
...
services:
  sub2api:
    image: registry.example.com/sub2api-custom:v0.1.122-2dev.1
```
...
```bash
tar czf backups/sub2api-files-$(date +%F-%H%M%S).tgz \
...
docker compose -f docker-compose.local.yml -f docker-compose.override.yml exec -T postgres \
  ...
  > backups/sub2api-db-$(date +%F-%H%M%S).sql
```
...
```bash
registry.example.com/sub2api-custom:v0.1.121-2dev.1
```
...
services:
  sub2api:
    image: registry.example.com/sub2api-custom:v0.1.121-2dev.1
```
...
- 镜像 tag 包含官方版本和二开构建号。
- 镜像已推送到自己的镜像仓库。
- 服务器 compose 使用自定义镜像，不使用 `weishaw/sub2api:latest`。
